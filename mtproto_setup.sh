...
#           MTProto Proxy — РАБОЧИЙ СКРИПТ v3
# ═══════════════════════════════════════════════════════════
...
SECRET=$(docker run --rm "$IMAGE" generate-secret tls -c "$FAKE_DOMAIN" 2>/dev/null | tr -d '[:space:]')
[ -z "$SECRET" ] && die "Не удалось сгенерировать секрет!"
